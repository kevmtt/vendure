...
class RequestContextService {
    createDefaultContext(config: Partial<ConstructorParameters<typeof RequestContext>[0]>) => Promise<RequestContext>;
    create(config: {
        req?: Request;
        apiType: ApiType;
...
<MemberInfo kind="method" type={`(config: Partial<ConstructorParameters<typeof <a href='/reference/typescript-api/request/request-context#requestcontext'>RequestContext</a>>[0]>) => Promise<<a href='/reference/typescript-api/request/request-context#requestcontext'>RequestContext</a>>`}  since="3.6.3"  />
