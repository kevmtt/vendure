...
class RequestContextService {
    createDefaultContext() => Promise<RequestContext>;
    create(config: {
        req?: Request;
        apiType: ApiType;
...
<MemberInfo kind="method" type={`() => Promise<<a href='/reference/typescript-api/request/request-context#requestcontext'>RequestContext</a>>`}  since="3.6.3"  />
...
`languageCode` & `currencyCode` derived from the default Channel's configuration.
### create
